...
show_help() {
    ...
    echo
    echo "Options:"
    echo "  -u, --update-only   Regenerate simulation file lists and exit"
    echo "      --no-update     Run a test without regenerating simulation file lists"
    ...
    echo "                      If omitted, runs all tests"
    echo
    echo "Examples:"
    echo "  $0 spi_tb               # Run spi_tb test"
    echo "  $0 --update-only        # Regenerate simulation file lists only"
    echo "  $0 --no-update spi_tb   # Run with existing simulation file lists"
...
if [ -n "$UPDATE_ONLY" ] && { [ -n "$NO_UPDATE" ] || [ -n "$TEST_NAME" ] || [ -n "$LINT" ] || [ -n "$EFX_MAP" ] || [ -n "$VIEW_WAVE" ]; }; then
    echo "Error: --update-only cannot be combined with other options or a test name"
    exit 1
fi

if [ -n "$NO_UPDATE" ] && { [ -z "$TEST_NAME" ] || [ -n "$LINT" ] || [ -n "$EFX_MAP" ] || [ -n "$VIEW_WAVE" ]; }; then
    echo "Error: --no-update requires exactly one test name"
    exit 1
fi
...
# Public test invocations go through CTest. Its fixture regenerates the shared
# file lists once before launching parallel --no-update workers.
if [ -z "$NO_UPDATE" ]; then
    if [ -n "$TEST_NAME" ] && [ ! -f "$PROJ_DIR/sim/${TEST_NAME}.sv" ]; then
    ...
    cd "$SCRIPT_DIR" && ctest "${CTEST_ARGS[@]}"
    CTEST_EXIT_CODE=$?
